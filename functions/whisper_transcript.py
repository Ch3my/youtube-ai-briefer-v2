...
async def whisper_transcript(video_url, output_dir=None):
        ...
        await send_message(
            {
                "action": "message",
                "msgCode": "info",
                "msg": f"Procesando audio {index+1}/{len(audio_segments)}",
            }
        )
        with open(segment, "rb") as file_handle:
            transcription = client.audio.transcriptions.create(
                model="whisper-1", file=file_handle, response_format="text"
            )
